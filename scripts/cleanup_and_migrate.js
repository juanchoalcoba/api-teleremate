...
async function run() {
  try {
    ...
    if (soldIds.length > 0) {
      ...
      console.log(
        `🗑️ Se borraron ${deleteResult.deletedCount} artículos vendidos en base a los JSON.`,
      );
    }
    ...
    if (unsoldStats > 0) {
      ...
      console.log(
        `✅ Se actualizó el status de ${statusResult.modifiedCount} artículos de 'upcoming' a 'depot' (Venta Directa).`,
      );
    } else {

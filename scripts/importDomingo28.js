...
async function importDomingo28() {
  try {
    const filePath = path.join(__dirname, "domingo28junio.json");
    if (!fs.existsSync(filePath)) {
      console.error("❌ No se encontró el archivo domingo28junio.json en backend/scripts/");
      process.exit(1);
    }

    const catalog = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!Array.isArray(catalog)) {
      console.error("❌ El formato de domingo28junio.json debe ser un array");
      process.exit(1);
    }

    console.log(`\n📦 Sincronizando ${catalog.length} artículos de domingo28junio.json...`);

    await mongoose.connect(MONGODB_URI);
    console.log("✅ Conectado a MongoDB");

    const targetDate = new Date("2026-06-28T00:00:00.000Z");
    console.log("🧹 Limpiando artículos previos con fecha 28 de Junio de 2026...");
    ...
    console.log(`\n📊 Resumen de Importación (domingo28junio.json):`);

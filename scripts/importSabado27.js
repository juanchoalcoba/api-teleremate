...
async function importSabado27() {
  try {
    const filePath = path.join(__dirname, "sabado27junio.json");
    if (!fs.existsSync(filePath)) {
      console.error("❌ No se encontró el archivo sabado27junio.json en backend/scripts/");
      process.exit(1);
    }

    const catalog = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!Array.isArray(catalog)) {
      console.error("❌ El formato de sabado27junio.json debe ser un array");
      process.exit(1);
    }

    console.log(`\n📦 Sincronizando ${catalog.length} artículos de sabado27junio.json...`);

    await mongoose.connect(MONGODB_URI);
    console.log("✅ Conectado a MongoDB");

    const targetDate = new Date("2026-06-27T00:00:00.000Z");
    console.log("🧹 Limpiando artículos previos con fecha 27 de Junio de 2026...");
    ...
    console.log(`\n📊 Resumen de Importación (sabado27junio.json):`);

...
async function run() {
  try {
    ...
    const file = path.join(__dirname, "..", "vendidos28junio.json");
    ...
    // Pasar lo que queda de 28 de junio a venta directa
    const unsoldStats = await Article.countDocuments({ 
      category: "remate", 
      auctionDate: new Date("2026-06-28T00:00:00.000Z") 
    });
    console.log(
      `\n🔄 Artículos del 28 de junio restantes para pasar a venta directa: ${unsoldStats}`,
    );

    if (unsoldStats > 0) {
      const resultCategory = await Article.collection.updateMany(
        { category: "remate", auctionDate: new Date("2026-06-28T00:00:00.000Z") },
        {
          $set: {
            category: "deposito",
            auctionLot: "",
      ...
      console.log("No hay artículos en remate del 28 de junio para migrar a venta directa.");
    }

...
async function processSold() {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      console.error(
        "❌ No se encontró el archivo vendidos.json en el directorio backend",
      );
      process.exit(1);
    }
    ...
    if (!Array.isArray(soldLotNumbers)) {
      console.error(
        "❌ El formato de vendidos.json debe ser un array de strings",
      );
      process.exit(1);
    }
    ...
    console.log(
      `   - Artículos encontrados y actualizados: ${result.modifiedCount}`,
    );
    console.log(
      `   - Artículos que no estaban en 'Próximo Remate' o no existen: ${soldLotNumbers.length - result.modifiedCount}`,
    );
    ...
    console.log(
      `   - Artículos que ahora están marcados como 'Vendido': ${alreadySold}`,
    );

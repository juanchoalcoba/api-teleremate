...
async function importDomingo31() {
  try {
    const filePath = path.join(__dirname, "../../domingo31.json");
    if (!fs.existsSync(filePath)) {
      console.error("❌ No se encontró el archivo domingo31.json en backend/");
      process.exit(1);
    }

    const catalog = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!Array.isArray(catalog)) {
      console.error("❌ El formato de domingo31.json debe ser un array");
      process.exit(1);
    }

    console.log(`\n📦 Sincronizando ${catalog.length} artículos de domingo31.json...`);

    await mongoose.connect(MONGODB_URI);
    console.log("✅ Conectado a MongoDB");

    const targetDate = new Date("2026-05-31T00:00:00.000Z");
    console.log("🧹 Limpiando artículos previos con fecha 31 de Mayo de 2026...");
    ...
    console.log(`\n📊 Resumen de Importación (domingo31.json):`);

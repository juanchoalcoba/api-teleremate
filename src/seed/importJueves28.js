...
async function importJueves28() {
  try {
    const filePath = path.join(__dirname, "../../jueves28.json");
    if (!fs.existsSync(filePath)) {
      console.error("❌ No se encontró el archivo jueves28.json en backend/");
      process.exit(1);
    }

    const catalog = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!Array.isArray(catalog)) {
      console.error("❌ El formato de jueves28.json debe ser un array");
      process.exit(1);
    }

    console.log(`\n📦 Sincronizando ${catalog.length} artículos de jueves28.json...`);

    await mongoose.connect(MONGODB_URI);
    console.log("✅ Conectado a MongoDB");

    // Opcional: limpiar artículos previos del 28 de mayo si los hubiera (para evitar duplicados)
    const targetDate = new Date("2026-05-28T00:00:00.000Z");
    console.log("🧹 Limpiando artículos previos con fecha 28 de Mayo de 2026...");
    ...
    console.log(`\n📊 Resumen de Importación (jueves28.json):`);

...
async function importViernes29() {
  try {
    const filePath = path.join(__dirname, "../../viernes29.json");
    if (!fs.existsSync(filePath)) {
      console.error("❌ No se encontró el archivo viernes29.json en backend/");
      process.exit(1);
    }

    const catalog = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (!Array.isArray(catalog)) {
      console.error("❌ El formato de viernes29.json debe ser un array");
      process.exit(1);
    }

    console.log(`\n📦 Sincronizando ${catalog.length} artículos de viernes29.json...`);

    await mongoose.connect(MONGODB_URI);
    console.log("✅ Conectado a MongoDB");

    const targetDate = new Date("2026-05-29T00:00:00.000Z");
    console.log("🧹 Limpiando artículos previos con fecha 29 de Mayo de 2026...");
    ...
    console.log(`\n📊 Resumen de Importación (viernes29.json):`);
